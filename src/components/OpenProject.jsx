...
const OpenProject = ({ id }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await instance.get(
          `https://eunhjxzv6b.execute-api.us-east-1.amazonaws.com/user/${id}`
        );
  ...
  const handleReject = async () => {
    try {
      const response = await instance.post(`/users/${userData?.userId}/reject`);
      if (response.status === 200) {
        console.log("User accepted successfully");
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <Flex px="4rem" gap={20} h="100%">
        <Flex w={"35%"}>
          <Image src={userData?.image_url} alt="rob" maxH="25rem" />
        </Flex>
        <Flex
          borderRadius="lg"
          fontSize="24px"
        ...
        >
          ...
          <Flex
            ...
            gap={15}
          >
            <Flex alignItems={"left"} direction={"column"} mt="1.5rem" gap={4}>
              <Flex gap={10}>
                <Flex>
                  <Text
                    width="100%"
              ...
              <Flex gap={10}>
                ...
                <Flex>
                  <Text
                    ...
                    WebkitBoxOrient="vertical"
                  >
                    Experience Id : {userData?.experience_id}
                  </Text>
                </Flex>
              </Flex>
            </Flex>
            <Flex gap={4}>
              <Text fontSize={"3xl"} color={"#5E17EB"} fontWeight={"bold"}>
                Rating :
              </Text>
              <Text fontSize={"3xl"} color={"#5E17EB"}>
                {parseFloat(userData?.average_rating).toFixed(1)}
              </Text>
            </Flex>
            <Flex
